...
echo -e " ›SUPPORT: Ubuntu 18.04/20.04/22.04 & Debian 10/11/12"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2

if [[ $( uname -m | awk '{print $1}' ) == "aarch64" || $( uname -m | awk '{print $1}' ) == "x86_64" ]]; then
...
# Instalasi Dasar Ruby & Wondershaper
apt update -y
apt install ruby -y
gem install lolcat
...
function base_package() {
    ...
    # Trik pintar menangani perbedaan nama Python antara Ubuntu 18.04, 20.04, dan 22.04
    sudo apt-get install -y python3
    sudo apt-get install -y python-is-python3 || true
    sudo apt-get install -y python || true

    # Instalasi paket utama (mendukung lintas versi Ubuntu/Debian)
    sudo apt-get install -y speedtest-cli vnstat libnss3-dev libnspr4-dev pkg-config libpam0g-dev libcap-ng-dev libcap-ng-utils libselinux1-dev libcurl4-openssl-dev flex bison make libnss3-tools libevent-dev bc rsyslog dos2unix zlib1g-dev libssl-dev libsqlite3-dev sed dirmngr libxml-parser-perl build-essential gcc g++ htop lsof tar wget curl ruby zip unzip p7zip-full python3-pip libc6 util-linux build-essential msmtp-mta ca-certificates bsd-mailx iptables iptables-persistent netfilter-persistent net-tools openssl gnupg gnupg2 lsb-release shc cmake git xz-utils apt-transport-https gnupg1 dnsutils jq openvpn easy-rsa
...
print_install "Memulai Instalasi Xaillaz"
instal
