...
# REPO - DIGANTI MENJADI xyoruz/scriptvpn
...
function base_package() {
    ...
    sudo apt-get install -y debconf-utils speedtest-cli vnstat libnss3-dev libnspr4-dev pkg-config libpam0g-dev libcap-ng-dev libcap-ng-utils libselinux1-dev libcurl4-nss-dev flex bison make libnss3-tools libevent-dev bc rsyslog dos2unix zlib1g-dev libssl-dev libsqlite3-dev sed dirmngr libxml-parser-perl build-essential gcc g++ python htop lsof tar wget curl ruby zip unzip p7zip-full python3-pip libc6 util-linux build-essential msmtp-mta ca-certificates bsd-mailx iptables iptables-persistent netfilter-persistent net-tools openssl gnupg gnupg2 lsb-release shc cmake git xz-utils apt-transport-https gnupg1 dnsutils jq openvpn easy-rsa
...
EOF
    ...
    curl -sS "ipinfo.ioecho -e " PROSES PENGECEKAN IP ADDRESS ANDA !!"
echo -e "${purple}----------------------------------------------------------${NC}"
echo -e " ›AUTHOR : ${green}Xaillaz ${NC}${YELLOW}(${NC}${green}V 3.2${NC}${YELLOW})${NC}"
echo -e " ›TEAM : Xaillaz STORE ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
...
# HAPUS BAGIAN IZIN SCRIPT DI SINI
MYIP=$(curl -sS ipv4.icanhazip.com)
clear
apt install ruby -y
gem install lolcat
...
# HAPUS SISTEM IZIN SC - langsung set username random
username=$(openssl rand -base64 12)
echo "$username" >/usr/bin/user
expx=$(date -d "+30 days" +"%Y-%m-%d")
echo "$expx" >/usr/bin/e
